    ...
    <b-container>
      ...
      <b-row class="mb-4">
        ...
        <b-col>
          ...
          ><i class="fas fa-plus-square"></i> ADICIONAR PATROCINADOR</router-link>
                <router-link
                    :to="{name:'editSponsor'}"
                    tag="button"
                    class="btn btn-outline-success mr-2 mt-2"
                ><i class="fas fa-edit"></i> EDITAR PATROCINADOR</router-link>
                  <button
          ...
          <table class="table table-striped">
            ...
            <tbody>
              <tr v-for="sponsor of sponsors" :key="sponsor._id">
                <td  class="pt-4">{{sponsor.sponsor}}</td>
...
import {
  FETCH_SPONSORS,
  REMOVE_SPONSOR //falta colocar no store
...
export default {
  ...
  methods: {
    ...
    compareNames(u1,u2) {
      if(u1.sponsor > u2.sponsor) return 1 * this.sortType
      else if(u1.sponsor < u2.sponsor) return -1 * this.sortType
      else return 0
    },
    viewSponsor(id) {
      ...
      this.$fire({
        ...
        html: <p><b>Animais:</b> ${this.sponsor.animals}</p>
        /* imageUrl: user.name,
        imageWidth: 400,
        imageHeight: 200,
    ...
    generateTemplate(sponsor) {
      let response = `
          <p><b>Animais:</b> ${this.sponsor.animals}</p>`          
